...
export class Forge {
    ...
    async initTheme(name) {
        ...
        await fs.writeFile(path.join(this.themePath, 'theme.json'), JSON.stringify(themeJson, null, 2));
        await fs.writeFile(path.join(this.themePath, 'config', 'settings_data.json'), JSON.stringify({ current: {} }, null, 2));
        console.log(chalk.green(`✓ Created theme structure at ${this.themePath}`));
    }
    async addComponent(typeStr) {
        const type = typeStr;
